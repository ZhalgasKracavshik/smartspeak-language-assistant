                                    ...
                                    subtitles.map((sub, idx) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`p-4 rounded-xl transition-all duration-300 cursor-pointer border ${isActive
                                                    ? 'bg-gradient-to-r from-blue-600 to-purple-600 border-white/20 shadow-xl scale-[1.02] active-karaoke-line'
                                                    : 'bg-white/5 border-transparent hover:bg-white/10 text-gray-400'
                                                    }`}
                                            >
                                                <div className="flex items-start gap-3">
                                                    <span className={`text-[10px] font-mono mt-0.5 whitespace-nowrap ${isActive ? 'text-white/70' : 'text-gray-500'}`}>
                                                        {Math.floor(sub.start_time / 60)}:{(sub.start_time % 60).toFixed(0).padStart(2, '0')}
                                                    </span>
                                                    <div className="space-y-1">
                                                        <p className={`text-sm leading-relaxed font-semibold ${isActive ? 'text-white' : 'text-gray-300'}`}>
                                                            {sub.text_en}
                                                        </p>
                                                        {(sub.text_ru || sub.text_kz) && (
                                                            <p className={`text-xs ${isActive ? 'text-white/80' : 'text-gray-500'} italic`}>
                                                                {sub.text_ru || sub.text_kz}
                                                            </p>
                                                        )}
                                                    </div>
