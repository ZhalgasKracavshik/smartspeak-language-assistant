-- ===================================================================
-- GRADE 9 CONTENT - PART 1: MODULES 1-3
-- Deletes old vocabulary and populates modules 1-3 with textbook content
-- ===================================================================

-- Step 1: Clean up old vocabulary from modules 1-3
...
FROM (VALUES
  ...
  ('excavate', 'проводить раскопки', 'қазбалар жүргізу'),
...
-- Module 3 Vocabulary - Section 3c (Tree Houses/Cappadocia)
INSERT INTO module_vocabulary (word, translation_ru, translation_kz, module_id, section)
SELECT 
  word, translation_ru, translation_kz,
  (SELECT id FROM modules WHERE title = 'Earth & our place on it' AND grade_level = 9 LIMIT 1),
  '3c'
FROM (VALUES
  ('tree top', 'верхушка дерева', 'ағаш басы'),
  ('thatched roof', 'соломенная крыша', 'сабан шатыр'),
  ('notch', 'зарубка', 'кесік'),
  ('mosquito', 'москит', 'маса'),
  ('disease', 'болезнь', 'ауру'),
  ('nightfall', 'сумерки', 'кеш'),
  ('chat', 'болтать', 'әңгімелесу'),
  ('fairy chimney', 'песчаная пирамида', 'құм пирамидасы'),
  ('ridge', 'хребет', 'жота'),
  ('upside-down', 'вверх ногами', 'төңкеріліп'),
  ('cone', 'конус', 'конус'),
  ('stone', 'камень', 'тас'),
  ('save', 'спасать', 'құтқару'),
  ('valley', 'долина', 'алқап'),
  ('courtyard', 'внутренний дворик', 'ауласы'),
  ('fairy-tale', 'сказочный', 'ертегі')
) AS v(word, translation_ru, translation_kz);

-- Module 3 Vocabulary - Section 3e (Thames River/London)
INSERT INTO module_vocabulary (word, translation_ru, translation_kz, module_id, section)
SELECT 
  word, translation_ru, translation_kz,
  (SELECT id FROM modules WHERE title = 'Earth & our place on it' AND grade_level = 9 LIMIT 1),
...
FROM (VALUES
  ('flow', 'течь', 'ағу'),
  ('iconic', 'культовый', 'символдық'),
  ('source', 'источник', 'бастау'),
  ('stroll', 'прогулка', 'серуен'),
  ('sunset', 'закат', 'күн батысы'),
  ('cable car', 'канатная дорога', 'арқанды жол'),
  ('drift', 'дрейфовать', 'жүзу'),
  ('in the blink of an eye', 'в мгновение ока', 'көз ілмей'),
  ('bank', 'берег реки', 'жағалау'),
  ('magnificent', 'великолепный', 'керемет'),
  ('artificial', 'искусственный', 'жасанды')
) AS v(word, translation_ru, translation_kz);

-- Module 3 Vocabulary - Section 3f (Mountain Formation)
INSERT INTO module_vocabulary (word, translation_ru, translation_kz, module_id, section)
SELECT 
  word, translation_ru, translation_kz,
  (SELECT id FROM modules WHERE title = 'Earth & our place on it' AND grade_level = 9 LIMIT 1),
  '3f'
FROM (VALUES
  ('form', 'формировать', 'қалыптастыру'),
  ('crust', 'кора', 'қабық'),
  ('tectonic plate', 'тектоническая плита', 'тектоникалық плита'),
  ('collide', 'сталкиваться', 'соғылу'),
  ('pressure', 'давление', 'қысым'),
  ('crack', 'трещина', 'жарық'),
  ('calculate', 'вычислять', 'есептеу'),
  ('melted', 'расплавленный', 'балқыған')
) AS v(word, translation_ru, translation_kz);

-- ===================================================================
-- COMPLETION MESSAGE
-- ===================================================================
SELECT 'Part 1 (Modules 1-3) completed successfully!' AS status;
